/**
 * see https://github.com/matthiasferch/tsm/blob/master/src/Vector3.ts
 * 
 */
...
export class Vector3 {
    ...
    multiplyByMat3(matrix: mat3, dest?: Vector3): Vector3 {
        if (!dest) { dest = this }

        return matrix.multiplyVec3(this, dest)
    }

    multiplyByQuat(quaternion: quat, dest?: Vector3): Vector3 {
        if (!dest) { dest = this }

        return quaternion.multiplyVec3(this, dest)
    }

    toQuat(dest?: quat): quat|undefined {
        if (!dest) { dest = new quat(0.0, 0.0, 0.0, 0.0) }
