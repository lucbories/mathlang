...
import { IIcNodeKindOf, ICompilerIcNode, ICompilerIcOperand,
    ICompilerIcFunction, ICompilerIcInstruction,
    ICompilerIcFunctionEnter, ICompilerIcFunctionLeave, ICompilerIcConstant
 } from '../../core/icompiler_ic_node';

import { ICompilerAstNode, ICompilerAstTypedNode , ICompilerAstBlockNode, ICompilerAstFunctionNode,
    ICompilerAstExpressionNode
 } from '../../core/icompiler_ast_node';
...
export default abstract class MathLangAstToIcVisitorStatements extends MathLangAstToIcVisitorBase{
    ...
    abstract visit_expression(ast_expression:any):ICompilerIcOperand|ICompilerError;
    abstract visit_value_id(ast_expression:any):ICompilerIcOperand|ICompilerError;
     ...
     */
    visit_function(compiler_function:ICompilerFunction):void {
		// GET FUNCTION AST NODE
		const ast_expression:ICompilerAstFunctionNode = <ICompilerAstFunctionNode>compiler_function.get_ast_node();
		/*
			{
     ...
     */
    visit_statements(ast_statements:ICompilerAstNode[]):void {
    ...
    visit_statement(ast_statement:ICompilerAstNode):void {
         switch(ast_statement.ast_code){
            ...
            case AST.STAT_RETURN:{
                this.visit_return_statement(<ICompilerAstTypedNode>ast_statement);
                return;
            }
     ...
     */
    visit_return_statement(ast_statement:ICompilerAstTypedNode):void {
        const ast_expression:ICompilerAstExpressionNode = <ICompilerAstExpressionNode>ast_statement;
        const ic_expression = this.visit_expression(ast_expression.expression);

        if (this.test_if_error(ic_expression)){
            this.add_error(ast_expression, 'Error in return expression:not a valid expression');
            return;
        }

        const ic_return = CompilerIcNode.create_function_return(this.get_compiler_scope(), ast_statement.ic_type, ic_expression);

        this.add_current_function_statement(ic_return);
    }
     ...
     * @param ast_statement AST statement
     */
    visit_if_statement(ast_statement:ICompilerAstNode):void {
        // BUILD THEN LABEL
        ...
        const ic_condition_opd:ICompilerIcNode = this.visit_expression(ast_statement.condition);
        ...
        this.get_current_function().set_ic_label_index(then_label, undefined);
