/**
 * see https://github.com/matthiasferch/tsm/blob/master/src/Matrix4.ts
 * 
 */

import { Matrix3 as mat3 } from './matrix3';
import { Vector3 as vec3 } from './vector3';
import { Vector4 as vec4 } from './vector4';
...
export class Matrix4 {
    ...
    constructor(values?: float[]) {
        if (values !== undefined) {
            this.init(values)
        }
    }

    private _values = FloatArrayCreate(16);

    static readonly identity = new Matrix4().setIdentity();

    private _zero = new vec3( [0, 0, 0] );
    private _one = new vec3( [1, 1, 1] );
    ...
    rotate(angle: number, axis: vec3): Matrix4|undefined {
        let x = axis.x
        let y = axis.y
        let z = axis.z
    ...
    static lookAt(position: vec3, target: vec3, up: vec3 = vec3.up): Matrix4 {
    ...
    make_rotation_from_quaternion(q:Quaternion){
